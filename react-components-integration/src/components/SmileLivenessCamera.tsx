import type { HTMLSmileLivenessElement, Configuration } from '@innovatrics/dot-smile-liveness';
...
function SmileLivenessCamera(configuration: Configuration) {
  useEffect(() => {
    // 2. Init existing custom web-component
    const smileLivenessHTMLElement =
      (document.getElementById('x-dot-smile-liveness') as HTMLSmileLivenessElement) || null;

    if (smileLivenessHTMLElement) {
      smileLivenessHTMLElement.configuration = configuration;
    }
  });
